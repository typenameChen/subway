...
class TAB_PAIR//更新表与原表配对情况类
{
public:
    enum STATUS{ALL,ONLY_ORI,ONLY_BK,NONE};//完整配对、只有原表、只有更新表、全无
    string ori;//原表名
    string bk;//更新表名
    STATUS status;//配对情况
    TAB_PAIR();//complete
...
private:
    ...
    bool IsBk(const string&tab_name);//是否更新表  complete
public:
    ...
    void Cover();//首先判断更新表或原表是否丢失，有丢失则抛出异常，否则更新表格覆盖原表格,注意本方法不选择数据库  complete
    ...
    SW_DATABASE(const string&username,const string&password,const string&use_db1, const string&use_db2);//登陆与设置数据库，默认使用use_db1数据库  complele
    ~SW_DATABASE();
};
